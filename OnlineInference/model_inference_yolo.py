...
for c_time, tmp_time in enumerate(partial_inference_video):
    ...
    while success:
        if (c >= video_t_start*fps) & (c<=video_t_end*fps):
            ...
            violation_frame , violation_id ,img_stack , bbox_stack = save_violation(bbox_stack,match_bbox,violation_frame,violation_id,img_stack,img_result,frame_num)
            ################## 存取違規的frame前後區間 並進行車牌辨識 ##########################
