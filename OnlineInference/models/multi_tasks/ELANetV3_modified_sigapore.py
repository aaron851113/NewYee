...
class MobileNet(nn.Module):
    def __init__(self, widen_factor=1.0, num_classes=1000, pretrained_path='/mnt/83a7cab6-2970-47cf-b4ae-9e770da2cb65/AIRD-PyTorch-Model-Dev-Platform/saved/mobilenet/mobilenet_1.pth'):
        """ Constructor
        Args:
        ...
        # shorten the channel from 1024 to 512
        self.dw5_6_ = block(512 * widen_factor, 512 * widen_factor, stride=2)
        self.dw6_ = block(512 * widen_factor, 512 * widen_factor)

        # self.avgpool = nn.AdaptiveAvgPool2d(1)
        # self.fc = nn.Linear(int(1024 * widen_factor), num_classes)

        # initial the model weight
...
def mobilenet(widen_factor=1.0, num_classes=1000):
    ...
    if os.path.isfile(model.pretrained_path):
        checkpoint_backbone = torch.load(model.pretrained_path)
        # pretrained_dict = checkpoint_backbone['state_dict']
        pretrained_dict = checkpoint_backbone
        ...
        print("loaded the pretrained weights from {}".format(model.pretrained_path))
    return model
########################################################################################################################
