...
class ESPNet_corner_heatmap(nn.Module):
    '''
    This class defines the ESPNet corner heatmap network
    '''
    ...
    def __init__(self, classes=20, p=2, q=3, encoderFile=None):
        ...
        self.up_l2 = nn.Sequential(nn.ConvTranspose2d(2*classes, classes, 2, stride=2, padding=0, output_padding=0, bias=False), BR(classes))
    ...
    def forward(self, input):
        ...
        output1_C = self.level3_C(output1_cat) # project to C-dimensional space
        # comb_l2_l3 = self.up_l2(self.combine_l2_l3(torch.cat([output1_C, output2_c], 1))) #RUM
        comb_l2_l3 = self.up_l2(torch.cat([output1_C, output2_c], 1)) #RUM
