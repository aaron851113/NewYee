...
from src.violation_plate import save_violation, createFolder, match_bbox_tracker
...
for c_time, tmp_time in enumerate(partial_inference_video):
    ...
    while success:
        if (c >= video_t_start*fps) & (c<=video_t_end*fps):
            ...
            #match_bbox_tracker = [decision,x1,y1,x2,y2,ob_id,cls]
            ...
            print("image stack length :",len(img_stack))
            violation_frame , violation_id , violation_id_satck ,img_stack , bbox_stack = save_violation(bbox_stack,match_bbox,cfg,charnet,violation_frame,violation_id,violation_id_satck,img_stack,img_result,frame_num)
            ################## 存取違規的frame前後區間 並進行車牌辨識 ##########################
